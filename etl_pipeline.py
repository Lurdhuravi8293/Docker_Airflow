# E-Commerce Data ETL Pipeline using PySpark
...
# Step 2: Load CSVs - absolute paths
ecom_df = spark.read.option("header", True).option("inferSchema", True) \
    .csv(r"C:\Users\ravi\OneDrive\Desktop\Pyspark_project\ecommerce_transactions.csv")

region_df = spark.read.option("header", True).option("inferSchema", True) \
    .csv(r"C:\Users\ravi\OneDrive\Desktop\Pyspark_project\country_region.csv")
...
monthly_pivot = ecom_enriched.groupBy("category", "month") \
    ...
    .groupBy("category") \
    .pivot("month") \
    .sum("sum(order_value)")  # Aggregated pivot
monthly_pivot.show()
...
base_path = r"C:\Users\ravi\OneDrive\Desktop\Pyspark_project"

# Country revenue
country_revenue.write.mode("overwrite") \
    .parquet(f"{base_path}\\gold_parquet\\country_revenue", compression="gzip")

country_revenue.write.mode("overwrite") \
    .option("header", True) \
    .csv(f"{base_path}\\gold_csv\\country_revenue")

# Region summary
region_summary.write.mode("overwrite") \
    .parquet(f"{base_path}\\gold_parquet\\region_summary", compression="gzip")

region_summary.write.mode("overwrite") \
    .option("header", True) \
    .csv(f"{base_path}\\gold_csv\\region_summary")

# Top customer
top_customer.write.mode("overwrite") \
    .parquet(f"{base_path}\\gold_parquet\\top_customer", compression="gzip")

top_customer.write.mode("overwrite") \
    .option("header", True) \
    .csv(f"{base_path}\\gold_csv\\top_customer")

print("✅ Output written to /gold_parquet and /gold_csv")

# Step 13: Keep Spark UI alive for 2 minutes
